...
import com.chinacreator.csx.proxy.util.SfProxyConstants;
 ...
 */
@Controller
@RequestMapping(SfProxyConstants.PATH_PREFIX)
public class SfProxyController {

	@Autowired(required = false)
	private APIConfigurations apiconfigurations;

	private static final Logger LOGGER = LoggerFactory
			.getLogger(SfProxyController.class);
	...
	static {
		try {
			...
			pattern = Pattern.compile("^(.*" + SfProxyConstants.PATH_PREFIX
					+ ")(.*)$");

			// 为了兼容K8s环境变量配置,首先加载下划线风格属性Key
			schema = ConfigManager.getInstance().getConfig(
					SfProxyConstants.SCHEMA_PROPERTY_KEY_);
			if (StringUtils.isEmpty(schema)) {
				schema = ConfigManager.getInstance().getConfig(
						SfProxyConstants.SCHEMA_PROPERTY_KEY);
			}
			if (StringUtils.isEmpty(schema)) {
				schema = "http";
			}
			apikey = ConfigManager.getInstance().getConfig(
					SfProxyConstants.API_KEY_PROPERTY_KEY_);
			if (StringUtils.isEmpty(apikey)) {
				apikey = ConfigManager.getInstance().getConfig(
						SfProxyConstants.API_KEY_PROPERTY_KEY);
			}

			host = ConfigManager.getInstance().getConfig(
					SfProxyConstants.HOST_PROPERTY_KEY_);
			if (StringUtils.isEmpty(host)) {
				host = ConfigManager.getInstance().getConfig(
						SfProxyConstants.HOST_PROPERTY_KEY);
			}
			...
			authorization = ConfigManager.getInstance().getConfig(
					SfProxyConstants.AUTH_API_KEY_PROPERTY_KEY_);
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			String port = ConfigManager.getInstance().getConfig(
					SfProxyConstants.PORT_PROPERTY_KEY_);
			if (StringUtils.isEmpty(port)) {
				port = ConfigManager.getInstance().getConfig(
						SfProxyConstants.PORT_PROPERTY_KEY);
			}
			SfProxyController.port = Integer.valueOf(port);
		} catch (NumberFormatException e) {
			SfProxyController.port = 0;
		}
		if (valid) {
			LOGGER.info("API网关代理初始化完成,apikey:" + apikey + ",schema:" + schema
					+ ",host:" + host + ",port:" + port);
		}
	...
	public void proxy(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			// 2.发送请求
			ClientHttpRequest newRequest = factory.createRequest(uri,
					...
					try {
						...
						if (e instanceof RestException) {
							...
							LOGGER.error("自定义拦截器["
									...
									+ "]处理API请求时发生了错误", e);
							throw new RestException("自定义拦截器["
									+ interceptor.getClass().getName()
	...
	private void handleRequestHeaders(HttpServletRequest request,
		...
		if (StringUtils.isNotEmpty(apikey)) {
			targetHeaders.set(SfProxyConstants.API_KEY_HEADER, apikey);

			User user = (User) ((WebOperationContext) OperationContextHolder
					.getContext()).getUser();
			if (user != null && user.size() != 0) {
				Map<String, String> userMap = new HashMap<String, String>();
				userMap.put(SfProxyConstants.OPERATOR_HEADER_USERNAME_KEY,
						user.getRealname());
				userMap.put(SfProxyConstants.OPERATOR_HEADER_USERID_KEY,
						user.getId());
				String code = org.apache.commons.codec.binary.Base64
						.encodeBase64String(JSON.toJSONString(userMap)
								.getBytes(Charset.forName("UTF-8")));
				targetHeaders.set(SfProxyConstants.OPERATOR_HEADER, code);
			}
	 ...
	 */
	private void handleResponseHeaders(HttpHeaders remoteHeaders,
