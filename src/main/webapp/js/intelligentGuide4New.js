	...
	methods: {
		getData: function(themeId) {
			...
			$.ajax({
				...
				type: 'GET',
				url: 'proxy/csx/v1/theme/' + themeId + '/asks?page=0',
				dataType: 'json',
		 ...
		 */
		confirm: function() {
			this.postData($("#themeId").val(),$("#themeName").val(),this.value[0],this.value[1]);
		},
		/**
		 * 答题完成发送请求
		 */
		postData: function(themeId,themeName,areaCodeUp,areaCode) {
			...
			$.ajax({
				...
				success: function (data, type, request) {
					...
					window.location.href = _this.getBasePath() + "index/processGuidance4New.jsp?guideprocessId=" + result 
		...
		validData: function() {
			...
			if(this.value.length == 0){
				this.askValid.push("请选择区域<br>");
			}else{
				if(this.askValid.indexOf("请选择区域<br>") != -1){
					this.askValid.splice(this.askValid.indexOf("请选择区域<br>"),1);
				}
			}
